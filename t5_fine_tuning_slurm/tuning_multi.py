...
num_samples = 500  # 9540 seconds to finish 25K samples on 4 gpus
...
def data_preprocessing(
    ...
    batch_size = hparams["0__batch_size"] * max(1, torch.cuda.device_count())
...
def fine_tuning(
    ...
    *,
    ...
    train_dataloader = torch.load(data_prepoc_output_path / "training_data", map_location=torch.device("cuda:0"))
    val_dataloader = torch.load(data_prepoc_output_path / "validation_data", map_location=torch.device("cuda:0"))
...
def model_distillation(
    ...
    *,
    ...
    tokenizer = T5Tokenizer.from_pretrained(
        student_tokenizer_path, d_model=128, d_ff=512, d_kv=16, num_layers=1
    )
    student_config = T5Config.from_pretrained(
        student_model_path, d_model=128, d_ff=512, d_kv=16, num_layers=1
    )
    student_model = T5ForConditionalGeneration(student_config).to(device)
