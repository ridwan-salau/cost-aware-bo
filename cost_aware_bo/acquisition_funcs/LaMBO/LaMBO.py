...
from functions.processing_funcs import (
    get_gen_bounds,
    get_dataset_bounds,
    get_initial_data,
)
...
class LaMBO:
    ...
    def update_loss_estimators(self, loss, root, probs, arm_idx, sigma, H, acq_value):
        ...
        for height in range(1, H):
            ...
            for leaf_idx in range(node.leaf_ranges[0], node.leaf_ranges[1]):
                nominator += probs[leaf_idx] * np.exp(
                    -self.eta * (1 + sigma[height - 1]) * loss[leaf_idx][height - 1]
                )

            denominator = probs[node.leaf_ranges[0] : node.leaf_ranges[1] + 1].sum()

            print(
                f"To update losses, the nominator is {nominator} and the denominator is {denominator}"
            )
    ...
    def update_arm_probability(self, loss, probs, arm_idx, n_leaves):
        ...
        print(
            f"To update probabilities, the nominator is {nominator} and the denominator is {denominator}"
        )
        eps = 1e-5
        if denominator <= eps:
            denominator = 1
    ...
    def remove_invalid_partitions(
        self, input_bounds, probs, h_ind, n_leaves, n_stages, leaf_partitions
    ):
            ...
            for i in range(n_stages - 1):
                for stage_idx in h_ind[i]:
                    if invalid_partition[i] == 0:
                        input_bounds[0][stage_idx] = (
                            input_bounds[0][stage_idx] + input_bounds[1][stage_idx]
                        ) / 2.0
                    else:
                        input_bounds[1][stage_idx] = (
                            input_bounds[0][stage_idx] + input_bounds[1][stage_idx]
                        ) / 2.0
    ...
    def lambo_trial(self, trial_number, acqf, wandb, params=None):
        chosen_functions, h_ind, total_budget = (
            params["obj_funcs"],
            params["h_ind"],
            params["total_budget"],
        )
        ...
        partitions, last_stage_partition = self.build_partitions(
        ...
        X_tree, Y_tree, C_tree, C_inv_tree, init_cost, best_f = self.build_datasets(
            acqf, mset.leaves, trial_number, n_leaves, params
        )
        ...
        print(
            f"Initial Data has {X_tree[arm_idx].shape} points for {acqf} Trial {trial_number} with cost {init_cost}"
        )
        ...
        while cum_cost < total_budget:
            ...
            input_bounds, arm_idx = self.select_arm(
                root, leaf_bounds, probs, h, arm_idx, n_leaves
            )
            ...
            new_x, n_memoised, acq_value = lambo_iteration(
                X,
                Y,
                C,
                C_inv,
            ...
            loss = self.update_loss_estimators(
                loss, root, probs, arm_idx, sigma, H, acq_value
            )

            probs = self.update_arm_probability(loss, probs, arm_idx, n_leaves)

            global_input_bounds = self.remove_invalid_partitions(
                input_bounds, probs, h_ind, n_leaves, n_stages, mset.leaf_partitions
            )
        ...
        print(
            f"{acqf} Trial {trial_number} Final Data has {X_tree[arm_idx].shape} datapoints with best_f {best_f:0,.2f}"
        )
