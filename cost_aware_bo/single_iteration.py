...
def get_cost_models(X, C, iter, param_idx, bounds, acqf):
    ...
    for i in range(C.shape[1]):
        ...
        try:
            ...
            print(
                f"BEFORE LOGGING THE COSTS, EXCEPTION RAISED BECAUSE THE MINIMUM DATAPOINT IS = \
                {stage_cost.min().item()}, MAXIMUM FOR SOME REASON IS = {stage_cost.max().item()},\
                 SHAPE IS = {stage_cost.shape}, AND NUMBER OF NANS IS {torch.isnan(stage_cost.view(-1)).sum().item()}"
            )
...
def bo_iteration(
    ...
    if acqf_str == "RAND":
        ...
        if acqf_str in ["EEIPU", "CArBO", "EIPS", "MS_CArBO"]:
            cost_mll, cost_gp = get_cost_models(
                train_x, c, iter, params["h_ind"], bounds, acqf_str
            )

        cost_sampler = SobolQMCNormalSampler(
            sample_shape=params["cost_samples"], seed=params["rand_seed"]
        )
        acqf = EIPUVariants(
            acq_type=acqf_str,
            model=gp_model,
    ...
    new_x, n_memoised = optimize_acqf_by_mem(
        acqf=acqf,
        acqf_str=acqf_str,
        bounds=norm_bounds,
        iter=iter,
