...
def arguments():
    ...
    parser.add_argument("--obj-funcs", nargs="+", help="Objective functions", default=["branin2", "hartmann3", "beale2"])
    parser.add_argument("--decay-factor", type=float, help="Decay factor", default=1)
    parser.add_argument("--cost-types", nargs="+", help="Cost types", default=[2,1,3])
    parser.add_argument("--warm-up", type=float, help="Warm up", default=1e-2)
    parser.add_argument("--trial-num", type=int, help="Trial number")
    parser.add_argument("--exp-group", type=str, help="Group ID")
    parser.add_argument("--acqf", type=str, help="Acquisition function", choices=["EEIPU", "EIPU", "EI", "RAND"])
...
if __name__=="__main__":
    ...
    wandb.init(
        entity="cost-bo",
        project="cost-aware-bo",
        group=f"{args.exp_group}--acqf_{args.acqf}|-obj-func_{'-'.join(args.obj_funcs)}|-dec-fac_{args.decay_factor}"
                f"|-cost-typ_{'-'.join(list(map(str,args.cost_types)))}|-warm-up_{args.warm_up}",
        name=f"{time.strftime('%Y-%m-%d-%H%M%S')}-trial-number_{trial}",
        config=params
    )
