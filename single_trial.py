...
def print_iter_logs(trial_logs):
    ...
    print(
        "c(x) = ["
        + ", ".join("{:.3f}".format(val) for val in trial_logs["c(x)"][-1])
        + "]",
        end="\t\t",
    )
    print(
        "E[c(x)] = ["
        + ", ".join("{:.3f}".format(val) for val in trial_logs["E[c(x)]"][-1])
        + "]",
        end="\t\t",
    )
...
def bo_trial(trial_number, acqf, wandb, params=None):
    ...
    X, Y, C = get_initial_data(
        params["n_init_data"], bounds=input_bounds, seed=trial_number, params=params
    )
    ...
    for iteration in range(params["n_iters"]):
        ...
        new_x, n_memoised, E_c, E_inv_c, y_pred = bo_iteration(
            X,
            Y,
            C,
            bounds=bounds,
        ...
        log = dict(
            ...
            c_res=dict(
                zip(
                    ...
                    [abs(act - est) for act, est in zip(E_c, cost_copy)],
                )
            ),
